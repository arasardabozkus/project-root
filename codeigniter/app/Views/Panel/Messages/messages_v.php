                ...
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    <li class="nav-item menu-open">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <li class="nav-item">
                                <a href="<?php echo base_url('admin/messages/read_messages')?>" class="nav-link active">
                                    <i class="far fa-circle nav-icon"></i>
                                    <p>Mesajlar</p>
                                </a>
                            </li>
        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <div id = "pageContent">
                    ...
                    <div class="row">
                        <div class="col-4"></div>
                        <div class="col-4"><button type="button" class="btn btn-primary btn-sm" id="readButton"><i class="fa-brands fa-readme"></i></button></div>
                                    ...
                                    <?php foreach ($messages as $key=>$message):?>
                                        <?php if($message->isRead):?>
                                            <tr class="custom-selectable" id="<?=$key?>">
                                                ...
                                                <td id = "t<?=$key?>"><?=$message->name?></td>
                                                <td style="display: none" id="p<?=$key?>"><?=$message->id?></td>
                                                <td><?=$message->mail?></td>
                                                <td><?=$message->subject?></td>
                                                <td><?=$message->time?></td>
                                            </tr>
                                        <?php else:?>
                                            <tr class="custom-selectable" id="<?=$key?>" style="font-weight: bold">
                                                <th scope="row"><input type="checkbox" id="c<?=$key?>"></th>
                                                <td id = "t<?=$key?>"><?=$message->name?></td>
                                                <td style="display: none" id="p<?=$key?>"><?=$message->id?></td>
                                                <td><?=$message->mail?></td>
            ...
            $('#readButton').click(function (){
                for (let i =0;i<<?=count($messages)?>;i++){
                    if($('#c'+i).is(':checked')){
                        ...
                        $.ajax({
                            type:"GET",
                            url:"<?=base_url('admin/messages/set_read/')?>" + id,
                            async: false,
            ...
            $('#rmButton').click(function (){
                for (let i =0;i<<?=count($messages)?>;i++){
                    if($('#c'+i).is(':checked')){
                        let title = $('#t'+i).text();
                        let id = $('#p'+i).text();
                        if(confirm(title + "'tan gelen mesaj silinecek, onaylıyor musunuz?")){
                            $.ajax({
                                type: "GET",
                                url:"<?=base_url('admin/messages/delete_message/')?>"+id,
                                success:function (){
                                    location.reload();
                                }
                            })
